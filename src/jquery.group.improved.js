(function() {
  (function($) {
      ...
      return {
        teams: state.participants.value().map(function(team) {
          ...
          return {
            id: team.id,
            name: team.name,
            format: (ref = team.format) != null ? ref : "",
            data: (ref1 = team.data) != null ? ref1 : {}
          };
        }),
        matches: state.matches.map(function(match) {
          return {
            a: {
              team: match.a.team.id,
              score: match.a.score
            },
            b: {
              team: match.b.team.id,
              score: match.b.score
            },
            round: match.round,
            time: match.time,
    ...
    standingsScoreColumnMarkup = '<td>{{wins}}</td> <td>{{losses}}</td> <td>{{ties}}</td> <td>{{points}}</td>';
    standingsViewTemplate = Handlebars.compile('<div class="standings"> <table> <colgroup> <col style="width: 50%"> <col span="5" style="width: 10%"> </colgroup> <tr><th></th><th>胜</th><th>负</th><th>平</th><th>积分</th></tr> {{#each this}} <tr data-teamid="{{team.id}}"><td>{{#if team.label}}{{team.label}}{{else}}{{team.name}}{{/if}}</td>' + standingsScoreColumnMarkup + '</tr> {{/each}} </table> </div>');
    standingsEditTemplate = Handlebars.compile('<div class="standings"> <table> <colgroup> <col style="width: 40%"> <col span="6" style="width: 10%"> </colgroup> <tr><th></th><th>胜</th><th>负</th><th>平</th><th>积分</th><th></th></tr> {{#each this}} <tr><td><input class="name" type="text" data-prev="{{team.name}}" data-teamid="{{team.id}}" value="{{team.name}}" /></td>' + standingsScoreColumnMarkup + '<td class="drop" data-name="{{team.id}}" title="Drop team">&#x2A2F;</td></tr> {{/each}} <tr><td><input class="add" type="text" /></td><td colspan="6"><input type="submit" value="Add" disabled="disabled" /></td></tr> </table> </div>');
    //roundTemplate = Handlebars.compile('<div data-roundid="{{this}}" class="round"> {{#if this}} <header>Round {{this}}</header> {{else}} <header>Unassigned</header> {{/if}} </div>');
    roundTemplate = Handlebars.compile('<div data-roundid="{{this}}" class="round"> {{#if this}} <header></header> {{else}} <header></header> {{/if}} </div>');
    matchViewTemplate = Handlebars.compile('<div data-matchid="{{id}}" class="match" draggable="{{draggable}}" alt="{{time}}"> <div class="team" data-teamid="{{a.team.id}}"> <div class="label">{{a.team.label}}</div> <div class="score {{homeClass}}">{{a.score}}</div> </div> <div class="team" data-teamid="{{b.team.id}}"> <div class="score {{awayClass}}">{{b.score}}</div> <div class="label">{{b.team.label}}</div> </div> </div>');
    ...
    roundsHeaderTemplate = Handlebars.compile('<header class="roundsHeader">详情</header>');
    ...
    initGenerateNewMatchId = function(pairs) {
      return localMatchCounter = pairs.size() > 0 ? pairs.max("id").value().id : 0;
    }
    localTeamCounter = 0;
      ...
      resultUpdates = matchProp.sampledBy(resultStream, function(propertyValue, streamValue) {
        ...
        propertyValue.matches = propertyValue.matches.map(function(it) {
          if (it.a.team.id === streamValue.a.team.id && it.b.team.id === streamValue.b.team.id) {
            ...
            it.time = streamValue.time;
            it.id = streamValue.id;
            it.a.team = streamValue.a.team;
            it.b.team = streamValue.b.team;
      ...
      init: function(opts) {
        ...
        if (opts.init) {
          ...
          pairs = _(opts.init.matches).map(function(it) {
            it.a.team = function(teams, id){
                for (var i = 0; i < teams.length; i++)
                {
                    if (teams[i].id == id)
                        return teams[i];
                }
            }(opts.init.teams, it.a.team);
            it.b.team = function(teams, id){
                for (var i = 0; i < teams.length; i++)
                {
                    if (teams[i].id == id)
                        return teams[i];
                }
            }(opts.init.teams, it.b.team);
            return it;
          }).sortBy(function(it) {
            //这里进行排序没有用
            return it.time;
